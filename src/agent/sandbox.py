...
class UnsupportedResourceLimitError(RuntimeError):
    """Raised when a requested sandbox limit cannot be enforced correctly."""
...
class AgentSandbox:
    ...
    def apply_limits(self, agent_id: str, limits: ResourceLimits) -> None:
        if limits.disk_mb is not None:
            raise UnsupportedResourceLimitError(
                "ResourceLimits.disk_mb is not enforced by AgentSandbox. "
                "Use an external filesystem quota or leave disk_mb unset."
            )
        if resource is None:
            return
        try:
